...
static void gpio_manager_arm_wakeup_interrupt(gpio_num_t gpio) {
    if(!initialized) return;
    esp_err_t err = gpio_wakeup_enable(gpio, GPIO_INTR_LOW_LEVEL);
    if(err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to enable gpio as sleep source : %s", esp_err_to_name(err));
    }
}

static void gpio_manager_disarm_wakeup_interrupt(gpio_num_t gpio) {
    if(!initialized) return;
    esp_err_t err = gpio_wakeup_disable(gpio);
    if(err != ESP_OK) {
...
void gpio_manager_init(void) {
    ...
    configure_active_mode_pin_unparking();
    ...
    if(err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to configure LBO pin : %s", esp_err_to_name(err));
    }
    ...
    configure_light_sleep_pin_parking();

    initialized = true;
    gpio_manager_arm_wakeup_interrupt(ENCODER_KEY_OK);
    /* --------------------- Add after soldering battery -----------------*/
    // gpio_manager_arm_wakeup_interrupt(SYSTEM_PIN_LBO);
...
void gpio_manager_deinit(void) {
    if(!initialized) return;
    gpio_manager_disarm_wakeup_interrupt(ENCODER_KEY_OK);
    /* --------------------- Add after soldering battery -----------------*/
    // gpio_manager_disarm_wakeup_interrupt(SYSTEM_PIN_LBO);
